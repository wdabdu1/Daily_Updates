...
if st.session_state.user_role == "Manager":
    settings_menu = st.sidebar.radio(
        ...
        ["Account Master", "Business Units (BU)", "Banks", "FX Currencies", "User Management"]
    )
else:
    ...
    if settings_menu == "User Management":
        ...
        with st.form("add_user"):
            ...
            if st.form_submit_button("Create User"):
                if new_u and new_p:
                    with engine.connect() as conn:
                        conn.execute(
                            text("INSERT INTO users (username, password_hash, role) VALUES (:u, :p, :r) ON CONFLICT DO NOTHING"),
        ...
        with st.form("add_bu"):
            ...
            if st.form_submit_button("Add BU"):
                if bu_name:
                    with engine.connect() as conn:
                        conn.execute(text("INSERT INTO bus (name) VALUES (:n) ON CONFLICT DO NOTHING"), {"n": bu_name})
                        conn.commit()
        ...
        with st.form("add_bank"):
            ...
            if st.form_submit_button("Add Bank"):
                if bs and bn:
                    with engine.connect() as conn:
                        conn.execute(text("INSERT INTO banks (short_name, full_name) VALUES (:s, :f) ON CONFLICT DO NOTHING"), {"s": bs, "f": bn})
                        conn.commit()
        ...
        with st.form("add_curr"):
            ...
            if st.form_submit_button("Add Currency"):
                if code:
                    with engine.connect() as conn:
                        conn.execute(text("INSERT INTO currencies (code) VALUES (:c) ON CONFLICT DO NOTHING"), {"c": code})
                        conn.commit()
        ...
        with st.form("add_acc"):
            c1, c2 = st.columns(2)
            selected_bu = c1.selectbox("Business Unit", bus if bus else ["Default BU"])
            dept = c2.text_input("Department")
            
            c3, c4 = st.columns(2)
            bank_choice = c3.selectbox("Bank", banks["short_name"].tolist() if not banks.empty else ["Default Bank"])
            ...
            if st.form_submit_button("Save Account"):
                fn = banks[banks["short_name"] == bank_choice]["full_name"].values[0] if not banks.empty else bank_choice
                with engine.connect() as conn:
                    conn.execute(text("""
                    ...
                    """), {"bu": selected_bu, "dept": dept, "bs": bank_choice, "bn": fn, "an": acc_num, "ana": acc_name, "curr": curr})
                    conn.commit()
...
if "Dashboard" in main_tab:
    ...
    cash_df = pd.read_sql("SELECT SUM(cash_balance) as total_cash FROM daily_cash_positions WHERE position_date = CURRENT_DATE", engine)
    dues_df = pd.read_sql("SELECT SUM(amount) as total_dues FROM bank_dues WHERE status = 'Active'", engine)

    total_cash = cash_df.iloc[0]["total_cash"] if not cash_df.empty and cash_df.iloc[0]["total_cash"] else 0.0
    total_dues = dues_df.iloc[0]["total_dues"] if not dues_df.empty and dues_df.iloc[0]["total_dues"] else 0.0
    ...
    if total_dues > total_cash:
        st.error(f"⚠️ Liquidity Alert: Bank Dues exceed available Cash in Hand by ${abs(gap):,.2f}!")
    else:
    ...
    if st.session_state.user_role in ["Read/Write", "Manager"]:
        ...
        from_c = c2.selectbox("From", all_currs, index=all_currs.index("AED") if "AED" in all_currs else 0)
        to_opts = [c for c in all_currs if c != from_c]
        to_c = c3.selectbox("To", to_opts, index=to_opts.index("SDG") if "SDG" in to_opts else 0)
    ...
    pairs = pd.read_sql("SELECT DISTINCT currency_pair FROM exchange_rates", engine)
    if not pairs.empty:
        sel_pair = st.selectbox("Select Pair", pairs["currency_pair"].tolist())
        rates_df = pd.read_sql(
            text("SELECT * FROM exchange_rates WHERE currency_pair = :p ORDER BY rate_date ASC"),
            engine,
            params={"p": sel_pair}
        )
        if not rates_df.empty:
            fig = px.line(rates_df, x="rate_date", y="rate", title=f"{sel_pair} Movement")
            st.plotly_chart(fig, use_container_width=True)

elif "Bank Dues & Cash" in main_tab:
    st.title("🏦 Bank Dues & Daily Cash Positions")
    acc_df = pd.read_sql("SELECT id, bank_shortname, account_number, currency FROM master_accounts", engine)
    acc_opts = {f"{r['bank_shortname']} - {r['account_number']} ({r['currency']})": r['id'] for _, r in acc_df.iterrows()} if not acc_df.empty else {}
            ...
            if acc_opts:
                with st.form("due_f"):
                    ...
                    if st.form_submit_button("Save Due"):
                        with engine.connect() as conn:
                            conn.execute(text("INSERT INTO bank_dues (account_id, due_date, facility_type, amount) VALUES (:a, :d, :f, :amt)"),
                                         {"a": acc_opts[choice], "d": d_date, "f": fac, "amt": amt})
                            conn.commit()
