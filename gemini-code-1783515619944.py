...
if menu == "Master Settings":
    ...
    with st.form("add_account"):
        st.subheader("Add Business Unit & Bank Account")
        c1, c2 = st.columns(2)
        ...
        currency = c7.selectbox("Currency", ["USD", "EUR", "EGP", "GBP", "SAR"])
...
# ---------------------------------------------------------
    ...
    # Get yesterday's rate for default fill
    last_rate_df = pd.read_sql(
        "SELECT rate FROM exchange_rates ORDER BY rate_date DESC LIMIT 1",
        engine,
    )
    default_rate = (
        float(last_rate_df.iloc[0]["rate"])
        if not last_rate_df.empty
        else 1.0000
    )

    with st.form("rate_entry"):
        st.subheader("Log Daily Rate")
        c1, c2, c3 = st.columns(3)
        rate_date = c1.date_input("Date", today)
        pair = c2.text_input("Currency Pair", "USD/EGP")
        rate_val = c3.number_input(
            "Rate", value=default_rate, format="%.4f", step=0.0001
        )

        if st.form_submit_button("Record Rate"):
            with engine.connect() as conn:
                conn.execute(
                    text("""
                    INSERT INTO exchange_rates (rate_date, currency_pair, rate, is_auto_filled)
                    VALUES (:d, :p, :r, FALSE)
                    ON CONFLICT (rate_date) DO UPDATE SET rate = EXCLUDED.rate, is_auto_filled = FALSE
                """),
                    {"d": rate_date, "p": pair, "r": rate_val},
                )
                conn.commit()
    ...
    rates_df = pd.read_sql(
        "SELECT * FROM exchange_rates ORDER BY rate_date ASC", engine
    )
    if not rates_df.empty:
        ...
        tf = st.selectbox(
            ...
            ["Year-to-Date", "Monthly", "Quarterly", "All Time"],
        )
...
# ---------------------------------------------------------
    ...
    acc_df = pd.read_sql("SELECT id, bank_shortname, account_number FROM master_accounts", engine)
    acc_options = {f"{r['bank_shortname']} - {r['account_number']}": r['id'] for _, r in acc_df.iterrows()} if not acc_df.empty else {}
    ...
    with t1:
        if not acc_options:
            ...
            with st.form("dues_form"):
                acc_choice = st.selectbox("Select Account", list(acc_options.keys()))
                due_date = st.date_input("Due Date")
                fac_type = st.text_input("Facility Type (e.g., LC, LTR, Overdraft)")
                amount = st.number_input("Amount Due", min_value=0.0, format="%.2f")
                ...
                if st.form_submit_button("Save Bank Due"):
                    with engine.connect() as conn:
                        conn.execute(
                            ...
                            VALUES (:a, :d, :f, :amt)
                        """),
                            {"a": acc_options[acc_choice], "d": due_date, "f": fac_type, "amt": amount},
                        )
                        conn.commit()
    ...
    with t2:
        if acc_options:
            with st.form("cash_form"):
                ...
                acc_choice = st.selectbox("Select Account", list(acc_options.keys()), key="cash_acc")
                cash_bal = st.number_input("Cash in Hand / Balance", min_value=0.0, format="%.2f")
                ...
                if st.form_submit_button("Record Position"):
                    with engine.connect() as conn:
                        ...
                        """),
                            {"d": pos_date, "a": acc_options[acc_choice], "c": cash_bal},
                        )
                        conn.commit()
